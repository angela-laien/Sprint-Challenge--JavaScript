...
zooAnimals.map(zooAnimal => {
  return lowCaseAnimalNames.push(zooAnimal.animal_name.toLowerCase())
});
...
zooAnimals.filter(zooAnimal => {
  ...
  return lowPopulationAnimals.push(zooAnimal);
  }
});
...
const populationTotal = zooAnimals.reduce((population, zooAnimal, index, zooAnimals) => {
  return population += zooAnimal.population;}, 0);
